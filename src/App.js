...
import DailyMoodTrackerContext from './components/DailyMoodTrackerContext'
...
class App extends Component {
  ...
  addEmojitoList = dateId => {
    const {activeEmoji, initialMonthList, selectedMonth} = this.state

    const selectedEmojiItem = emojisList.find(
      eachitem => eachitem.id === activeEmoji,
    )

    const selectedMonthArray = initialMonthList[selectedMonth]

    const selectedDateIndex = selectedMonthArray.dates.findIndex(
      eachitem => eachitem.id === dateId,
    )

    const selectedDateItem = selectedMonthArray.dates[selectedDateIndex]

    if (selectedEmojiItem.emojiUrl === selectedDateItem.emojiUrl) {
      const newDateItem = {
      ...
      initialMonthList[selectedMonth].dates.splice(
        selectedDateIndex,
        1,
        newDateItem,
      )
      ...
      const newDateItem = {
        id: selectedDateItem.id,
        date: selectedDateItem.date,
        emojiUrl: selectedEmojiItem.emojiUrl,
        emojiName: selectedEmojiItem.emojiName,
      }
      initialMonthList[selectedMonth].dates.splice(
        selectedDateIndex,
        1,
    ...
    this.setState({initialMonthList}, () => {
      this.calculateDayWiseCount()
    })
  }
  ...
  changeEmoji = event => {
    ...
    this.setState({selectedEmoji: selectedEmojiName}, () => {
      this.calculateDayWiseCount()
    })
  }
  ...
  onclickEmojiActive = id => {
    this.setState({activeEmoji: id})
  }

  /*  ///////////////////////////////////////////////////////////////////////////////////////////////////////// */

  getOverallEmojiCount = emojiName => {
    const {initialMonthList} = this.state

    let count = 0
    initialMonthList.forEach(item => {
      const datesItem = item.dates
      datesItem.forEach(dateItem => {
        if (dateItem.emojiName === emojiName) {
          count += 1
        }
      })
    })
  ...
  getRequiredGraphData = emojiName => {
    const {selectedGraphMonth, initialMonthList} = this.state

    const selectedMonthData = initialMonthList.findIndex(
      eachitem => eachitem.monthName === selectedGraphMonth,
    )

    let count = 0
    initialMonthList[selectedMonthData].dates.forEach(eachitem => {
      if (eachitem.emojiName === emojiName) {
        count += 1
      }
    })
  ...
  changeGraphMonth = event => {
    this.setState({selectedGraphMonth: event.target.value})
  }
  ...
  calculateDayWiseCount = () => {
    const {selectedEmoji, selectedDay} = this.state

    const {initialMonthList, selectedMonth} = this.state

    const selectedMonthList = initialMonthList[selectedMonth]

    const {dates} = selectedMonthList
    ...
    dates.forEach(eachItem => {
      if (
        parseInt(eachItem.date) % 7 === parseInt(selectedDay) &&
        eachItem.emojiName === selectedEmoji
      ) {
        dayCount += 1
        dayWiseArray.push(eachItem)
      }
    })
    console.log(dayWiseArray)

    const requiredCOunt = `0${dayCount}`
    this.setState({dayWiseCount: requiredCOunt})
  }
  ...
  render() {
    ...
    return (
      <DailyMoodTrackerContext.Provider
        value={{
          initialDaysList,
          initialMonthList,
          emojiList,
          addEmojitoList: this.addEmojitoList,
          selectedDay,
          selectedMonth,
          selectedEmoji,
          selectedGraphMonth,
          ...
          activeEmoji,
          dayWiseCount,
          onclickEmojiActive: this.onclickEmojiActive,
          onClickRightArrow: this.onClickRightArrow,
          onClickLeftArrow: this.onClickLeftArrow,
          getOverallEmojiCount: this.getOverallEmojiCount,
          getRequiredGraphData: this.getRequiredGraphData,
          changeGraphMonth: this.changeGraphMonth,
      ...
      >
        <Switch>
          ...
          <ProtectedRoute
            exact
            path="/reports"
            data-testid="reportsRoute"
            component={ReportsRoute}
          />
          <ProtectedRoute
            exact
            path="/not-found"
            data-testid="notfoundRooute"
